  ...
  dynamixel_sdk_examples::GetMoreMotors::Response & res)
{
  ...
  if (dxl_comm_result == COMM_SUCCESS) {
    ...
    position2 = groupSyncRead.getData((uint8_t)req.id3, ADDR_PRESENT_POSITION, 4);
...
void setMoreMotorsPositionCallback(const dynamixel_sdk_examples::SetMoreMotors::ConstPtr & msg)
{
  ...
  param_goal_position3[1] = DXL_HIBYTE(DXL_LOWORD(position2));
  ...
  if (dxl_comm_result == COMM_SUCCESS) {
    ...
    ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", msg->id2, msg->position3);
  } else {
